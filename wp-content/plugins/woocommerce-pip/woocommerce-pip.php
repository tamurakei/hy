...
if ( version_compare( get_option( 'woocommerce_db_version' ), '2.2', '<' ) ) {
	...
	function woocommerce_pip_outdated_version_notice() {
		...
		$message = sprintf(
			__( '%sWooCommerce Print Invoice/Packaging List is inactive.%s This version requires WooCommerce 2.2 or newer. Please %supdate WooCommerce to version 2.2 or newer%s', 'woocommerce-pip' ),
			'<strong>',
			'</strong>',
...
function woocommerce_pip_action_links( $links ) {
	...
	$plugin_links = array(
		'<a href="' . admin_url( 'admin.php?page=woocommerce_pip' ) . '">' . __( 'Configure', 'woocommerce-pip' ) . '</a>',
...
function woocommerce_pip_order_items_table( $order, $show_price = FALSE ) {
	...
	foreach( $order->get_items() as $item ) {
		...
		$item_meta = new WC_Order_Item_Meta( version_compare( get_option( 'woocommerce_db_version' ), '2.4', '<' ) ? $item['item_meta'] : $item );
